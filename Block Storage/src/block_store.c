...
size_t block_store_allocate(block_store_t *const bs){
    ...
    if (fzAddress == SIZE_MAX || fzAddress == 255) return SIZE_MAX;
...
bool block_store_request(block_store_t *const bs, const size_t block_id){
    ...
    if(bitmap_test(bs->bitmap, block_id) == 1) return 0;

    bitmap_set(bs->bitmap, block_id);

    if(bitmap_test(bs->bitmap, block_id) == 0) return 0;
...
/**
 * These next two functionns are for bonus
block_store_t *block_store_deserialize(const char *const filename){
    block_store_t* newBlockStore;
    return newBlockStore;
}
